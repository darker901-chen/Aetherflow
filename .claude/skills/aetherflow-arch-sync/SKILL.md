...
3. Filesystem inventory (use Glob, do NOT read every file):
...
### Mechanical (auto-apply with Edit tool)
...
In apply mode, the report should clearly mark `[APPLIED]` vs `[REPORT-ONLY]` for each drift. Auto-applied edits go through the normal Edit tool so the user sees them in `git diff`.
...
- Glob inventory should be under 2 seconds total (use Glob tool, don't `find`).

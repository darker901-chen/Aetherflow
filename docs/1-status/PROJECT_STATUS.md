...
| Deterministic notification/window masking | Implemented and enabled by default | **Fixture + Teams alias + live LINE verified** | The default whitelist covers LINE, Slack, Discord, Teams, Telegram, and WhatsApp executable identities. `line_window_mask_live_20260723` masked a real installed LINE window on 199/200 frames; `teams_notification_alias_fix` proves Teams alias/config/window identity. Canonical live artifacts are still missing for Slack, Discord, Telegram, and WhatsApp. |

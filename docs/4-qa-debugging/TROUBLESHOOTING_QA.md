...
## QA Coverage Registry (2026-07-23)
...
| Windows default smoke and trace | **Passed with warning** | `evidence_refresh_20260723`: 900/900 encoded, 0 encode failures, 0 parse errors; `capture_motion_20260723`: 0/300 retries with moving content | The static-desktop retry ratio (8.979) is a content-dynamics artifact, not a capture defect. Remaining: a longer moving-content soak and the retry-threshold hard-gate decision. |
...
| Notification/chat-window mask | **Fixture + Teams alias + live LINE verified** | Local Win32 fixture, `teams_notification_alias_fix`, and `line_window_mask_live_20260723` (real LINE window masked 199/200 frames) | Record canonical live Slack, Discord, Telegram, and WhatsApp runs. A baseline with no visible target is not evidence. |
| Windows visual mask modes | **Trace-verified per mode / human visual confirmation pending** | `mask_sweep_{blackout,blur,mosaic}_20260723`: 120/120 frames per mode through distinct GPU shader paths; muxed mp4s in `output/` | Human uninterrupted source-versus-output confirmation and the internal grayscale demo outcome remain. The scene demo historically exercised only two of five classes. |
...
| SRT output | **NVENC loopback verified on current HEAD** | `srt_loopback_refresh_20260723`: a local FFmpeg caller joined mid-GOP and decoded 90 live frames; sender logged 0 queue-full drops and returned to listen mode after client close | Test Intel/oneVPL, phone VLC over LAN, firewall onboarding, reconnect under packet loss, and non-local latency. Version 1 remains single-viewer and video-only. |
...
| Portable package | **Current-HEAD package verified via staged self-tests** | `AetherFlow-portable-20260723.zip` (44.8 MiB, sha `0536d42`): staged capture/encode run, SRT probe decoded 30 frames, Studio UI smoke exit 0; SHA256 recorded in PROJECT_STATUS | Exercise `--with-model`, test a clean Intel-only machine, and decide signing/SmartScreen/installer policy. |
| ONNX setup and classifier | **Runtime plumbing verified on current HEAD / accuracy unverified** | `scene_classifier_onnx_20260723`: DirectML provider, p95 inference 13.235 ms, 0 failures, all four classifier gates passed | Execute a clean dependency/model setup, label a real-screen sweep, measure accuracy, make the Stage A-to-B decision, and cover all five visual outcomes. |
...
| Formal release artifact | **Not created; release-candidate zip ready** | No tag or GitHub Release is claimed; a current-HEAD zip with SHA256 exists (2026-07-23) | Choose a version and tag, decide signing/support scope, then create the release with the recorded checksum. |
